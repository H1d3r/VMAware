...
*Ranges from 0 to 50%, where a high score indicates high specificity to VM environments.*
...
```Each category is assigned a score arbitrarily using the considerations as guidelines. Then, the scores are summe to get a final technique score. If the technique shows a high likelihood of false positives, the final score is reduced, potentially lowering the score to 5 in cases of high false flag likelihood.```
...
Penalty: No reduction applied to the score.
...
Penalty: Subtract 25% of the score.
...
Penalty: Subtract 50% of the score.
...
Penalty: Subtract 80% of the score. If the result is lower than 5, the final score is set to 5.